...
/* Uninstall the route from kernel. */
static void
rib_uninstall (struct route_node *rn, struct rib *rib, uint16_t virtualNetworkNo)
{
  if (CHECK_FLAG (rib->flags, ZEBRA_FLAG_SELECTED))
    {
      redistribute_delete (&rn->p, rib, virtualNetworkNo);
      if (! RIB_SYSTEM_ROUTE (rib))
...
static void
rib_process (struct route_node *rn, uint16_t virtualNetworkNo)
{
  struct rib *rib;
  struct rib *next;
  struct rib *fib = NULL;
  ...
  if (select && select == fib)
    {
      ...
      if (CHECK_FLAG (select->flags, ZEBRA_FLAG_CHANGED))
        {
          redistribute_delete (&rn->p, select, virtualNetworkNo);
          ...
          redistribute_add (&rn->p, select, virtualNetworkNo);
        }
  ...
  if (fib)
    {
      ...
      redistribute_delete (&rn->p, fib, virtualNetworkNo);
 ...
 */
static unsigned int
process_subq (struct list * subq, u_char qindex, uint16_t virtualNetworkNo)
{
  ...
  rib_process (rnode, virtualNetworkNo);
...
meta_queue_process (struct work_queue *dummy, void *data, uint16_t virtualNetworkNo)
{
  struct meta_queue * mq = data;
  unsigned i;

  for (i = 0; i < MQ_SIZE; i++)
    if (process_subq (mq->subq[i], i, virtualNetworkNo))
      {
	mq->size--;
	break;
      }
 ...
 */
void rib_lookup_and_pushup (struct prefix_ipv4 * p, uint16_t virtualNetworkNo)
{
  struct route_table *table;
  struct route_node *rn;
  struct rib *rib;
  ...
  for (rib = rn->info; rib; rib = rib->next)
  {
      ...
      ! RIB_SYSTEM_ROUTE (rib))
    {
      ...
      rib_uninstall (rn, rib, virtualNetworkNo);
    }
  }
  if (changed)
    rib_queue_add (&zebrad, rn);
...
static void
static_uninstall_ipv4 (struct prefix *p, struct static_ipv4 *si, uint16_t virtualNetworkNo)
{
  struct route_node *rn;
  struct rib *rib;
  struct nexthop *nexthop;
...
static_add_ipv4 (struct prefix *p, struct in_addr *gate, const char *ifname,
		 u_char flags, u_char distance, u_int32_t vrf_id, uint16_t virtualNetworkNo)
{
  u_char type = 0;
  struct route_node *rn;
  ...
  /* Distance changed.  */
  if (update)
    static_delete_ipv4 (p, gate, ifname, update->distance, vrf_id, virtualNetworkNo);
...
static_delete_ipv4 (struct prefix *p, struct in_addr *gate, const char *ifname,
		    u_char distance, u_int32_t vrf_id, uint16_t virtualNetworkNo)
{
  u_char type = 0;
  struct route_node *rn;
  struct static_ipv4 *si;
  ...
  /* Install into rib. */
  static_uninstall_ipv4 (p, si, virtualNetworkNo);
...
static void
static_uninstall_ipv6 (struct prefix *p, struct static_ipv6 *si, uint16_t virtualNetworkNo)
{
  struct route_table *table;
  struct route_node *rn;
  struct rib *rib;
...
static_add_ipv6 (struct prefix *p, u_char type, struct in6_addr *gate,
		 ...
		 u_int32_t vrf_id, uint16_t virtualNetworkNo)
{
  struct route_node *rn;
...
static_delete_ipv6 (struct prefix *p, u_char type, struct in6_addr *gate,
		    const char *ifname, u_char distance, u_int32_t vrf_id, uint16_t virtualNetworkNo)
{
  struct route_node *rn;
  struct static_ipv6 *si;
  struct route_table *stable;
  ...
  /* Install into rib. */
  static_uninstall_ipv6 (p, si, virtualNetworkNo);
