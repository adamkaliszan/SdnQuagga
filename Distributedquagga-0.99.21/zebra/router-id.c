...
{
  uint16_t virtualNetworkNo = 0; //TODO Adam dodać ustawianie
  struct prefix p2;
  struct listnode *node;
  struct zserv *client;
  ...
  for (ALL_LIST_ELEMENTS_RO (zebrad.client_list, node, client))
    zsend_router_id_update (client, &p2, virtualNetworkNo);
}

void
router_id_add_address (struct connected *ifc)
{
  uint16_t virtualNetworkNo = 0; //TODO Adam zrobić ustawianie
  struct list *l = NULL;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (zebrad.client_list, node, client))
    zsend_router_id_update (client, &after, virtualNetworkNo);
}
