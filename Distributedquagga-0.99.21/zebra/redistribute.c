...
static void
zebra_redistribute_default (struct zserv *client, uint16_t virtualNetworkNo)
{
      ...
      if (rn)
	{
	  for (newrib = rn->info; newrib; newrib = newrib->next)
		...
		&& newrib->distance != DISTANCE_INFINITY)
	      zsend_route_multipath (ZEBRA_IPV4_ROUTE_ADD, client, &rn->p, newrib, virtualNetworkNo);
  ...
  if (table)
      ...
      if (rn)
	{
	  for (newrib = rn->info; newrib; newrib = newrib->next)
	    if (CHECK_FLAG (newrib->flags, ZEBRA_FLAG_SELECTED)
		&& newrib->distance != DISTANCE_INFINITY)
	      zsend_route_multipath (ZEBRA_IPV6_ROUTE_ADD, client, &rn->p, newrib, virtualNetworkNo);
	  route_unlock_node (rn);
	}
    }
#endif /* HAVE_IPV6 */
...
/* Redistribute routes. */
static void
zebra_redistribute (struct zserv *client, int type, uint16_t virtualNetworkNo)
{
  struct rib *newrib;
  struct route_table *table;
  struct route_node *rn;
...
void
redistribute_add (struct prefix *p, struct rib *rib, uint16_t virtualNetworkNo)
{
      ...
      if (is_default (p))
        {
          if (client->redist_default || client->redist[rib->type])
            {
              if (p->family == AF_INET)
                zsend_route_multipath (ZEBRA_IPV4_ROUTE_ADD, client, p, rib, virtualNetworkNo);
#ifdef HAVE_IPV6
              if (p->family == AF_INET6)
                zsend_route_multipath (ZEBRA_IPV6_ROUTE_ADD, client, p, rib, virtualNetworkNo);
#endif /* HAVE_IPV6 */	  
	    }
        }
...
void
redistribute_delete (struct prefix *p, struct rib *rib, uint16_t virtualNetworkNo)
{
  struct listnode *node, *nnode;
  struct zserv *client;
  ...
  for (ALL_LIST_ELEMENTS (zebrad.client_list, node, nnode, client))
	...
	{
	  if (p->family == AF_INET)
	    zsend_route_multipath (ZEBRA_IPV4_ROUTE_DELETE, client, p, rib, virtualNetworkNo);
#ifdef HAVE_IPV6
	  if (p->family == AF_INET6)
	    zsend_route_multipath (ZEBRA_IPV6_ROUTE_DELETE, client, p, rib, virtualNetworkNo);
#endif /* HAVE_IPV6 */
	}
    }
}
...
void
zebra_redistribute_add (int command, struct zserv *client, int length, uint16_t virtualNetworkNo)
{
  int type;
  ...
  if (! client->redist[type])
    {
      ...
      zebra_redistribute (client, type, virtualNetworkNo);
    }
}

void
zebra_redistribute_delete (int command, struct zserv *client, int length, uint16_t virtualNetworkNo)
{
  int type;
...
void
zebra_redistribute_default_add (int command, struct zserv *client, int length, uint16_t virtualNetworkNo)
{
  client->redist_default = 1;
  zebra_redistribute_default (client, virtualNetworkNo);
}     

void
zebra_redistribute_default_delete (int command, struct zserv *client,
				   int length, uint16_t virtualNetworkNo)
{
