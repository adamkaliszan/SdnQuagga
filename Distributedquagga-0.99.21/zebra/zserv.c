...
{ 
  ...
  ZEBRA_READ, 
  ZEBRA_WRITE, 
  ZEBRA_READ_NET,         // Control Plane Adapter (Zebra) recived Control Plane (e.g. ospf hello message) packet
  ZEBRA_WRITE_NET         // Send Control Plane (e.g. OSPFd) packet via Control Plane Adapter (Zebra)
};
...
static void
zserv_create_header (struct stream *s, uint16_t cmd, uint16_t virtualNetworkNo)
{
  ...
  stream_putw (s, cmd);
  stream_putw (s, virtualNetworkNo);  
}
...
zsend_interface_add (struct zserv *client, struct interface *ifp)
{
  ...
  zserv_create_header (s, ZEBRA_INTERFACE_ADD, ifp->virtualNetworkNo);
...
zsend_interface_delete (struct zserv *client, struct interface *ifp)
{
  ...
  zserv_create_header (s, ZEBRA_INTERFACE_DELETE, ifp->virtualNetworkNo);
  zserv_encode_interface (s, ifp);
                         ...
                         struct interface *ifp, struct connected *ifc)
{
  ...
  zserv_create_header (s, cmd, ifp->virtualNetworkNo);
  stream_putl (s, ifp->ifindex);
...
zsend_route_multipath (int cmd, struct zserv *client, struct prefix *p,
                       struct rib *rib, uint16_t virtualNetworkNo)
{
  int psize;
  struct stream *s;
  ...
  zserv_create_header (s, cmd, virtualNetworkNo);
...
#ifdef HAVE_IPV6
static int
zsend_ipv6_nexthop_lookup (struct zserv *client, struct in6_addr *addr, uint16_t virtualNetworkNo)
{
  struct stream *s;
  struct rib *rib;
  unsigned long nump;
  ...
  /* Fill in result. */
  zserv_create_header (s, ZEBRA_IPV6_NEXTHOP_LOOKUP, virtualNetworkNo);
...
static int
zsend_ipv4_nexthop_lookup (struct zserv *client, struct in_addr addr, uint16_t virtualNetworkNo)
{
  struct stream *s;
  struct rib *rib;
  unsigned long nump;
  ...
  /* Fill in result. */
  zserv_create_header (s, ZEBRA_IPV4_NEXTHOP_LOOKUP, virtualNetworkNo);
...
static int
zsend_ipv4_import_lookup (struct zserv *client, struct prefix_ipv4 *p, uint16_t virtualNetworkNo)
{
  struct stream *s;
  struct rib *rib;
  unsigned long nump;
  ...
  /* Fill in result. */
  zserv_create_header (s, ZEBRA_IPV4_IMPORT_LOOKUP, virtualNetworkNo);
...
int
zsend_router_id_update (struct zserv *client, struct prefix *p, uint16_t virtualNetworkNo)
{
  struct stream *s;
  int blen;
  ...
  /* Message type. */
  zserv_create_header (s, ZEBRA_ROUTER_ID_UPDATE, virtualNetworkNo);
 ...
 */
static int
zread_ipv4_add (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  int i;
  struct rib *rib;
...
static int
zread_ipv4_delete (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  int i;
  struct stream *s;
...
static int
zread_ipv4_nexthop_lookup (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  struct in_addr addr;

  addr.s_addr = stream_get_ipv4 (client->ibuf);
  return zsend_ipv4_nexthop_lookup (client, addr, virtualNetworkNo);
}

/* Nexthop lookup for IPv4. */
static int
zread_ipv4_import_lookup (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  struct prefix_ipv4 p;
  ...
  return zsend_ipv4_import_lookup (client, &p, virtualNetworkNo);
}

#ifdef HAVE_IPV6
/* Zebra server IPv6 prefix add function. */
static int
zread_ipv6_add (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  int i;
  struct stream *s;
...
static int
zread_ipv6_delete (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  int i;
  struct stream *s;
  struct zapi_ipv6 api;
...
static int
zread_ipv6_nexthop_lookup (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  ...
  return zsend_ipv6_nexthop_lookup (client, &addr, virtualNetworkNo);
}
#endif /* HAVE_IPV6 */

/* Register zebra server router-id information.  Send current router-id */
static int
zread_router_id_add (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  struct prefix p;
  ...
  return zsend_router_id_update (client,&p, virtualNetworkNo);
}

/* Unregister zebra server router-id information. */
static int
zread_router_id_delete (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  client->ridinfo = 0;
  return 0;
}
...
static void
zread_hello (struct zserv *client, uint16_t virtualNetworkNo)
{
...
static int sendIPv6Msg(int fd, struct in6_addr *src, struct in6_addr *dst, unsigned int dstIfIdx, uint16_t virtualNetworkNo, struct iovec *tmpIovecMsg, int flags)
{
  struct msghdr smsghdr;
  struct cmsghdr *scmsgp;
  struct in6_pktinfo *pktinfo;
  
  int x = sizeof (struct in6_pktinfo);
  u_char cmsgbuf[CMSG_SPACE(x)];

  static struct sockaddr_in6 dst_sin6;
  ...
  /* source address */
  pktinfo->ipi6_ifindex = (unsigned int) dstIfIdx;
  if (src)
  ...
  //TODO sprawdzić, czy jest to potrzebne  
  //#ifdef HAVE_SIN6_SCOPE_ID
  dst_sin6.sin6_scope_id = dstIfIdx;
  //#endif
  ...
  smsghdr.msg_iov = tmpIovecMsg;
  smsghdr.msg_iovlen = iov_count (tmpIovecMsg);

  smsghdr.msg_control = (caddr_t) cmsgbuf;
  smsghdr.msg_controllen = sizeof (cmsgbuf);
  smsghdr.msg_flags = 0;
   
  int ret = sendmsg (fd, &smsghdr, flags);
  if (ret < 0)
  {
    static char strSrcAddr[47];
    static char strDstAddr[47];
  
    inet_ntop (AF_INET6, src, strSrcAddr, 46);
    inet_ntop (AF_INET6, &dst_sin6.sin6_addr, strDstAddr, 46);
    zlog_err("CP -> NET: Failed to forward IPv6 packet (%s -> %s) to network (errno: %s (%d), fd %d, ifIndex %d packet length %d)", 
	     strSrcAddr, strDstAddr, strerror(errno), errno, fd, dstIfIdx, tmpIovecMsg[0].iov_len);
  }
  return ret;
}

static int sendIPv4Msg(int fd, struct in_addr *src, struct in_addr *dst, unsigned int ifindex, uint16_t virtualNetworkNo, struct iovec *tmpIovecMsg, int flags)
{
  int x = sizeof (struct in_pktinfo);
  u_char cmsgbuf[CMSG_SPACE(x)];
  memset (cmsgbuf, 0, CMSG_SPACE(x));
  ...
  pktinfo = (struct in_pktinfo *)(cmsgbuf);
  
  
  pktinfo->ipi_ifindex = ifindex;
  if (src)
    pktinfo->ipi_addr = *src;
  else
  ...
  msg.msg_iov = tmpIovecMsg;
  msg.msg_iovlen = iov_count(tmpIovecMsg);// 2; 
  msg.msg_name = (caddr_t) &sa_dst;
  msg.msg_namelen = sizeof (sa_dst); 
  
  msg.msg_control = (caddr_t) cmsgbuf;
  msg.msg_controllen = 12; //CMSG_SPACE(x); //sizeof (cmsgbuf);  //FIXME
  msg.msg_flags = 0;
  ...
  if (ret < 0)
  {
    static char strSrcAddr[16];
    static char strDstAddr[16];
  
    int msgLen = 0;
    for (int i=0; i<msg.msg_iovlen; i++)
      msgLen += tmpIovecMsg[i].iov_len;
    
    inet_ntop (AF_INET, src, strSrcAddr, 15);
    inet_ntop (AF_INET, dst, strDstAddr, 15);
    zlog_err("CP -> NET: Failed to forward IPv4 packet (%s -> %s) to network %s (%d), fd %d, ifIndex %d packet length %d IOV count %d, flags %d", 
      strSrcAddr, strDstAddr, strerror(errno), errno, fd, ifindex, msgLen, msg.msg_iovlen, flags);
  }
  return ret;
}
...
static uint16_t ospfv2SumCalc(size_t lenOspfMsg, struct in6_addr *srcAddr, struct in6_addr *dstAddr, uint8_t *buffOspfMsg)
{
#define OSPFv3_CRC_OFFSET 12
  uint16_t prot_ospfv3=89;
  uint16_t end=lenOspfMsg;
...
static int
zread_txc (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  if (client->sock_net < 0)
  {
    zlog_err("%s DP socket doesn't exists.", __func__);
    return -1;
  }

  struct iovec tmpIovecMsg[3];
  tmpIovecMsg[1].iov_base = NULL;
  tmpIovecMsg[1].iov_len  = 0;
  tmpIovecMsg[2].iov_base = NULL;
  tmpIovecMsg[2].iov_len  = 0;

  unsigned char msgToTx[1500];
  


/**
 * Send protocol message via DP interface 
 ...
 */ 
  //TODO Adam add flags do sendmsg function, e.g. Don't route. Add new field in header to do that 
  int dstIfIdx = stream_getl(client->ibuf);

  stream_get((void *)(msgToTx), client->ibuf, length - 4);
  

       
  if (msgToTx[0] >> 4 == 6)
  {
    size_t MsgLen = length - 4 - sizeof(struct ip6_hdr);   

    tmpIovecMsg[0].iov_base = msgToTx + sizeof(struct ip6_hdr);
    tmpIovecMsg[0].iov_len  = MsgLen;
    
    struct ip6_hdr *ipHdd6 = (struct ip6_hdr*)(msgToTx);
    struct in6_addr *src = &ipHdd6->ip6_src;
    struct in6_addr *dst = &ipHdd6->ip6_dst; 
    
    uint16_t *chkSumPtr = (uint16_t *)(tmpIovecMsg[0].iov_base + 12);
    if (*chkSumPtr == 0)
    {
       ospfv3SumCalc (MsgLen, src, dst, (uint8_t *)(tmpIovecMsg[0].iov_base));
    }
    
    int ret = sendIPv6Msg(client->sock_net, src, dst, dstIfIdx, virtualNetworkNo, tmpIovecMsg, 0 /*flags */);
    if (ret >= 0)
    {
      char strDstAddr[47];
      char strSrcAddr[47];
      ...
      inet_ntop (AF_INET6, src, strSrcAddr, 46);
      inet_ntop (AF_INET6, dst, strDstAddr, 46);
      zlog_debug("CP -> NET: src %s, dst %s, VirtualNetworkNo %d, ifIndex %d len %d, result %d", strSrcAddr, strDstAddr, virtualNetworkNo, dstIfIdx, MsgLen, ret);
    }
    else
    {
      zlog_err("CP -> NET IPv6 packet forward failed");
    }
    return ret;
  }
  ...
  if (msgToTx[0] >> 4 == 4)
  {
    struct iphdr *tmpIphdr = msgToTx;
    int ip4hdrLen =  tmpIphdr->ihl * 4;
    size_t MsgLen = length - 4 -ip4hdrLen;   

    tmpIovecMsg[0].iov_base = msgToTx;
    tmpIovecMsg[0].iov_len  = ip4hdrLen;

    tmpIovecMsg[1].iov_base = msgToTx + ip4hdrLen;
    tmpIovecMsg[1].iov_len  = MsgLen;

    struct iphdr *ipHdd4 = (struct iphdr*)(msgToTx);
    struct in_addr *src = (struct in_addr *) &ipHdd4->saddr;
    struct in_addr *dst = (struct in_addr *) &ipHdd4->daddr; 

    //uint16_t *chkSumPtr = (uint16_t *)(tmpIovecMsg[1].iov_base + 12);
    //*chkSumPtr = 0;
    //     if (*chkSumPtr == 0)
    //     {
    //       zlog_debug("Calculating OSPFv2 is not calculated");
    //     //  ospfv2SumCalc (MsgLen, src, dst, (uint8_t *)(tmpIovecMsg[0].iov_base));
    //     }
      
    int ret = sendIPv4Msg(client->sock_net, src, dst, dstIfIdx, virtualNetworkNo, tmpIovecMsg, 0 /* flags */);
    if (ret >= 0)
    {
      char strDstAddr[17];
      char strSrcAddr[17];
      inet_ntop (AF_INET, src, strSrcAddr, 16);
      inet_ntop (AF_INET, dst, strDstAddr, 16);
      zlog_debug("CP -> NET: src %s, dst %s, VirtualNetworkNo %d, ifIndex %d len %d, result %d", strSrcAddr, strDstAddr, virtualNetworkNo, dstIfIdx, length - 4, ret);
    }
//     else
//     {
//       zlog_err("CP -> NET IPv4 packet forward failed");
//     }
    return ret;
  }
  zlog_err("zread_txc: unrecognized Layer3 protocol");
  return -3;
}

static int 
zread_sockopt (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  if (client->sock_net < 0)
  {
    zlog_err("%s DP socket doesn't exists.", __func__);
 ...
 *  96 |           ...                         |
 */
  int level   = stream_getl(client->ibuf);
  int optName = stream_getl(client->ibuf);
    
  socklen_t optLen = length - 8;   
  stream_get((void *)(msgToTx), client->ibuf, (size_t)optLen);
  
  int ret = setsockopt(client->sock_net, level, optName, msgToTx, optLen);
  if (ret < 0)
    zlog_err("SetSockOpt failed (level %d, opt. name %d, opt. len %d)", level, optName, optLen);
  else
    zlog_debug("SetSockOpt (level %d, opt. name %d, opt. len %d)", level, optName, optLen);
  
  return 0; 
}

static int
zread_createNetSck (struct zserv *client, u_short length, uint16_t virtualNetworkNo)
{
  if (client->sock_net > 0)
  {
    zlog_warn("Socket alrready exists");
    return client->sock_net;
  }
  ...
  if (length < 12)
  {
    zlog_err("%s Zły rozmiar pakietu %d", __func__, length);
    return -1;
  }
  else
  {
    client->sock_net_domain   = stream_getl(client->ibuf);
    client->sock_net_type     = stream_getl(client->ibuf);
    client->sock_net_protocol = stream_getl(client->ibuf);
  }
  client->sock_net = socket (client->sock_net_domain, client->sock_net_type, client->sock_net_protocol);

  if (client->sock_net == -1)
  {
    zlog_err("Creating Zebra DP socket failed (domain %d, type %d, protocol %d) ERRNO = %d (%s)", client->sock_net_domain, client->sock_net_type, client->sock_net_protocol, errno, safe_strerror (errno)); 
    return -1;
  }

  zlog_debug("Creating Zebra DP socket (domain %d, type %d, protocol %d) FD = %d", client->sock_net_domain, client->sock_net_type, client->sock_net_protocol, client->sock_net);       
  zebra_event (ZEBRA_READ_NET, client->sock_net, client); 
  return client->sock_net;
}
...
zebra_client_close (struct zserv *client)
{
  ...
  if (client->t_read_net)
    thread_cancel (client->t_read_net);
  if (client->t_write)
...
zebra_client_create (int sock)
{
  ...
  /* Buffer for receiving routing protocol packets from NETwork */
  client->message_net[0].iov_base = client->message_net_buffer;
  client->message_net[0].iov_len = ZEBRA_MAX_PACKET_SIZ;
  client->message_net[1].iov_base = NULL;
  client->message_net[1].iov_len = 0;
...
static int zebra_client_read_net_IPv6 (struct thread *thread)
{
  int sock;
  struct zserv *client;
  size_t already;
  ...
  ssize_t nbyte =  stream_read_try_IPv6(sock, &src, &dst, &ifindex, &virtualNetworkNo, client->message_net);
  
  inet_ntop (AF_INET6, &src, strSrc, 47);
  inet_ntop (AF_INET6, &dst, strDst, 47);
  
  zlog_debug("NET -> CP: scr %s, dst %s VirtualNetworkNo %d, ifindex %d, len %d", strSrc, strDst, virtualNetworkNo, ifindex, nbyte);
  
  struct stream *s = client->obuf;
  stream_reset(s); 

  zserv_create_header (s, ZEBRA_TXC, 0);
  stream_putl (s, ifindex);
  
  //Create IPv6 header
  struct ip6_hdr  tmpIPv6Hdr;
  tmpIPv6Hdr.ip6_ctlun.ip6_un1.ip6_un1_flow = htonl(6<<28);               /* 4  bits version, 8 bits TC, 20 bits flow-ID */
  tmpIPv6Hdr.ip6_ctlun.ip6_un1.ip6_un1_plen = htons(nbyte);               /* 16 bits */
  tmpIPv6Hdr.ip6_ctlun.ip6_un1.ip6_un1_nxt  = 89;                         /* 8  bits */
  tmpIPv6Hdr.ip6_ctlun.ip6_un1.ip6_un1_hlim = 64;                         /* 8  bits */
  memcpy(&tmpIPv6Hdr.ip6_src, &src, sizeof(struct in6_addr));
  memcpy(&tmpIPv6Hdr.ip6_dst, &dst, sizeof(struct in6_addr)); 
     
  stream_put(s, (void *)(&tmpIPv6Hdr), sizeof(struct ip6_hdr));  
  
  //IPv6 payload
  stream_put(s, client->message_net[0].iov_base, nbyte);  
  stream_putw_at (s, 0, stream_get_endp (s));
  
  stream_set_getp(s, 0);
  zebra_server_send_message(client);
  
  zebra_event (ZEBRA_READ_NET, sock, client);
  return 0;
}

/* Handler of zebra service request. */
static int zebra_client_read_net_IPv4 (struct thread *thread)
{
  int sock;
  struct zserv *client;
  size_t already;
  ...
  ssize_t nbyte =  stream_read_try_IPv4(sock, &src, &dst, &ifindex, &virtualNetworkNo, client->message_net);
  
  inet_ntop (AF_INET, &src, strSrc, 47);
  inet_ntop (AF_INET, &dst, strDst, 47);
...
zebra_client_read (struct thread *thread)
{
  ...
  struct zserv *client;
  size_t already;
  uint16_t length, command, virtualNetworkNo;
  ...
  virtualNetworkNo = stream_getw (client->ibuf);
  ...
  switch (command) 
    ...
    case ZEBRA_ROUTER_ID_ADD:
      zread_router_id_add (client, length, virtualNetworkNo);
      break;
    case ZEBRA_ROUTER_ID_DELETE:
      zread_router_id_delete (client, length, virtualNetworkNo);
      break;
    ...
    case ZEBRA_IPV4_ROUTE_ADD:
      zread_ipv4_add (client, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV4_ROUTE_DELETE:
      zread_ipv4_delete (client, length, virtualNetworkNo);
      break;
#ifdef HAVE_IPV6
    case ZEBRA_IPV6_ROUTE_ADD:
      zread_ipv6_add (client, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV6_ROUTE_DELETE:
      zread_ipv6_delete (client, length, virtualNetworkNo);
      break;
#endif /* HAVE_IPV6 */
    case ZEBRA_REDISTRIBUTE_ADD:
      zebra_redistribute_add (command, client, length, virtualNetworkNo);
      break;
    case ZEBRA_REDISTRIBUTE_DELETE:
      zebra_redistribute_delete (command, client, length, virtualNetworkNo);
      break;
    case ZEBRA_REDISTRIBUTE_DEFAULT_ADD:
      zebra_redistribute_default_add (command, client, length, virtualNetworkNo);
      break;
    case ZEBRA_REDISTRIBUTE_DEFAULT_DELETE:
      zebra_redistribute_default_delete (command, client, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV4_NEXTHOP_LOOKUP:
      zread_ipv4_nexthop_lookup (client, length, virtualNetworkNo);
      break;
#ifdef HAVE_IPV6
    case ZEBRA_IPV6_NEXTHOP_LOOKUP:
      zread_ipv6_nexthop_lookup (client, length, virtualNetworkNo);
      break;
#endif /* HAVE_IPV6 */
    case ZEBRA_IPV4_IMPORT_LOOKUP:
      zread_ipv4_import_lookup (client, length, virtualNetworkNo);
      break;
    case ZEBRA_HELLO:
      zread_hello (client, virtualNetworkNo);
      break;
    case ZEBRA_TXC:
      zread_txc(client, length, virtualNetworkNo);
      break;
    case ZEBRA_SOCKOPT:
      zread_sockopt(client, length, virtualNetworkNo);
      break;      
    case ZEBRA_CREATE_NET_SCK:
      zread_createNetSck(client, length, virtualNetworkNo);
      break;      
    default:
...
zebra_event (enum event event, int sock, struct zserv *client)
{
  switch (event)
  {
    ...
    case ZEBRA_WRITE:
      ...
      break;
    case ZEBRA_READ_NET:
      if ((client->sock_net_domain == AF_INET6) && (client->sock_net_type == SOCK_RAW) && (client->sock_net_protocol == IPPROTO_OSPFIGP))
      { 
        client->t_read_net = thread_add_read(zebrad.master, zebra_client_read_net_IPv6, client, sock);
        break;
      }
      if ((client->sock_net_domain == AF_INET) && (client->sock_net_type == SOCK_RAW) && (client->sock_net_protocol == IPPROTO_OSPFIGP))
      { 
        client->t_read_net = thread_add_read(zebrad.master, zebra_client_read_net_IPv4, client, sock);
        break;
      }
      
    case ZEBRA_WRITE_NET:
      /**/
      break;
  }
}
