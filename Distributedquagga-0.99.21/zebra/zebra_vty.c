		   ...
		   const char *flag_str, const char *distance_str)
{
  ...
  uint16_t virtualNetworkNo = 0; //TODO Adam Zrobić odczyt sieci wirtualnej. wymagana zmiana CLI
  ...
  if ((gate_str != NULL) && (strncasecmp (gate_str, "Null0", strlen (gate_str)) == 0))
    {
      ...
      if (add_cmd)
        static_add_ipv4 (&p, NULL, NULL, ZEBRA_FLAG_BLACKHOLE, distance, 0, virtualNetworkNo);
      else
        static_delete_ipv4 (&p, NULL, NULL, distance, 0, virtualNetworkNo);
      return CMD_SUCCESS;
    }
  ...
  {
    if (add_cmd)
      static_add_ipv4 (&p, NULL, NULL, flag, distance, 0, virtualNetworkNo);
    else
      static_delete_ipv4 (&p, NULL, NULL, distance, 0, virtualNetworkNo);
  ...
  if (add_cmd)
    static_add_ipv4 (&p, ifname ? NULL : &gate, ifname, flag, distance, 0, virtualNetworkNo);
  else
    static_delete_ipv4 (&p, ifname ? NULL : &gate, ifname, distance, 0, virtualNetworkNo);
...
static_ipv6_func (struct vty *vty, int add_cmd, const char *dest_str,
		  ...
		  const char *flag_str, const char *distance_str)
{
  uint16_t virtualNetworkNo;   //TODO Adam Odczytać tą wartość
  int ret;
  u_char distance;
  struct prefix p;
  ...
  if (add_cmd)
    static_add_ipv6 (&p, type, gate, ifname, flag, distance, table, virtualNetworkNo);
  else
    static_delete_ipv6 (&p, type, gate, ifname, distance, table, virtualNetworkNo);
