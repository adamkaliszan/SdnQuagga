...
ospf6_zebra_redistribute (int type)
{
  ...
  if (zclient->sock > 0)
    zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, type, 0); //TODO Adam zadbać o pozostałe instancje
}
...
ospf6_zebra_no_redistribute (int type)
{
  ...
  if (zclient->sock > 0)
    zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient, type, 0); //TODO Adam zadbać o pozostałe instancje
}


static int 
ospf6_zebra_txc (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct ospf6_interface *oi;
  struct ospf6_header *oh;
  
  unsigned int ifindex = stream_getl(zclient->ibuf);
  length -= (4 + sizeof(struct ip6_hdr));

  oi = ospf6_interface_lookup_by_ifindex (ifindex);
  
  int bytesToRead = STREAM_READABLE(zclient->ibuf);
  
  zlog_debug("CPA -> CP, ifindex %d dataLen %d (length %d)", ifindex, bytesToRead, length);
  
     
  //  zlog_debug("receive message from CPAT start");
  static char tmpRecBuf[1500];
  

  stream_get(tmpRecBuf, zclient->ibuf, bytesToRead);
  
  
   
  struct ip6_hdr  *tmpIp6hdr    = (struct ip6_hdr  *)(tmpRecBuf);
  oh = (struct ospf6_header *) (tmpRecBuf + sizeof(struct ip6_hdr));

  struct in6_addr src = tmpIp6hdr->ip6_src;
  struct in6_addr dst = tmpIp6hdr->ip6_dst;
  
  //inet_pton (AF_INET6, "2001:db8::4", &src);
  //inet_pton (AF_INET6, "2001:db8::5", &dst);

  
  return ospf6_process_recMsg(oi, oh, length, &src, &dst);
}


/* Inteface addition message from zebra. */
static int
ospf6_zebra_if_add (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct interface *ifp;

  ifp = zebra_interface_add_read (zclient->ibuf, virtualNetworkNo);
...
static int
ospf6_zebra_if_del (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct interface *ifp;

  if (!(ifp = zebra_interface_state_read(zclient->ibuf, virtualNetworkNo)))
    return 0;
...
ospf6_zebra_if_state_update (int command, struct zclient *zclient,
                             zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct interface *ifp;

  ifp = zebra_interface_state_read (zclient->ibuf, virtualNetworkNo);
  if (ifp == NULL)
    return 0;
...
ospf6_zebra_route_update (int type, struct ospf6_route *request)
{
  ...
  if (type == REM)
    ret = zapi_ipv6_route (ZEBRA_IPV6_ROUTE_DELETE, zclient, dest, &api, 0);//TODO Adam zadbać o pozostałe instancje
  else
    ret = zapi_ipv6_route (ZEBRA_IPV6_ROUTE_ADD, zclient, dest, &api, 0);//TODO Adam zadbać o pozostałe instancje
