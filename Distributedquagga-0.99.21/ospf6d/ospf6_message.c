...
#else
int
ospf6_process_recMsg(struct ospf6_interface *oi, struct ospf6_header *oh, int len, struct in6_addr *src, struct in6_addr *dst)
{
  if (oi == NULL || oi->area == NULL)
  {
    zlog_debug ("Message received on disabled interface");
            ...
            struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  if (len != ntohs (oh->length))
    zlog_err ("Could not send entire message (len %d != %d)", len, ntohs (oh->length));
}
