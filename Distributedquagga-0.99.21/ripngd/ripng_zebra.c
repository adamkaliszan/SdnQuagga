...
int ripng_interface_up (int, struct zclient *, zebra_size_t, uint16_t virtualNetworkNo);
int ripng_interface_down (int, struct zclient *, zebra_size_t, uint16_t virtualNetworkNo);
int ripng_interface_add (int, struct zclient *, zebra_size_t, uint16_t virtualNetworkNo);
int ripng_interface_delete (int, struct zclient *, zebra_size_t, uint16_t virtualNetworkNo);
  ...
  if (zclient->redist[ZEBRA_ROUTE_RIPNG])
    {
      ...
      zapi_ipv6_route (ZEBRA_IPV6_ROUTE_ADD, zclient, p, &api, 0); //TODO Adam zadbać o pozostałe instancje
    }
}
  ...
  if (zclient->redist[ZEBRA_ROUTE_RIPNG])
    {
      ...
      zapi_ipv6_route (ZEBRA_IPV6_ROUTE_DELETE, zclient, p, &api, 0); //TODO Adam zadbać o pozostałe instancje
    }
}
      ...
      if (zclient->redist[redist_type[i].type])
        {
          if (zclient->sock > 0)
            zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE,
                                     zclient, redist_type[i].type, 0); //TODO Adam zadbać o pozostałe instancje
