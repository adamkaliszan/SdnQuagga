...
static int
bgp_interface_add (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
		      ...
		      zebra_size_t length, uint16_t virtualNetworkNo)
{
  ...
  s = zclient->ibuf;
  ifp = zebra_interface_state_read (s, virtualNetworkNo);
...
static int
bgp_interface_up (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct stream *s;
  struct interface *ifp;
  struct connected *c;
...
static int
bgp_interface_down (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  struct stream *s;
  struct interface *ifp;
  struct connected *c;
  ...
  if (p->family == AF_INET)
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
      ...
      zapi_ipv4_route (ZEBRA_IPV4_ROUTE_ADD, zclient, 
                       (struct prefix_ipv4 *) p, &api, 0); //TODO Adam uzupełnić numer wirtualnej sieci
    }
#ifdef HAVE_IPV6
  ...
  if (p->family == AF_INET6)
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
      ...
      zapi_ipv6_route (ZEBRA_IPV6_ROUTE_ADD, zclient, 
                       (struct prefix_ipv6 *) p, &api, 0); //TODO Adam zadbać o pozostałe instancje
    }
#endif /* HAVE_IPV6 */
}
  ...
  if (p->family == AF_INET)
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
      ...
      zapi_ipv4_route (ZEBRA_IPV4_ROUTE_DELETE, zclient, 
                       (struct prefix_ipv4 *) p, &api, 0); //TODO Adam zadbać o pozostałe instancje
    }
#ifdef HAVE_IPV6
  /* We have to think about a IPv6 link-local address curse. */
  if (p->family == AF_INET6)
