...
#define OSPF_WRITE_FRAG_SHIFT 3
  ...
  while ( (stream_get_endp(op->s) - stream_get_getp (op->s)) > maxdatasize )
  {
    ...
    if (ret < 0)
      zlog_warn ("*** ospf_write_frags: sendmsg failed to %s, id %d, off %d, len %d, mtu %u failed with %s", inet_ntoa (iph->ip_dst), iph->ip_id, iph->ip_off, iph->ip_len, mtu, safe_strerror (errno));
...
static int ospf_sendmsg (int fd, struct in_addr *src, struct in_addr *dst, unsigned int ifindex, struct iovec *iov, int flags)
{
...
#else
static int ospf_sendmsg (struct in_addr *src, struct in_addr *dst, unsigned int ifindex, struct iovec *iov, int flags)
{
  struct ip *iph = (struct ip*)(iov[0].iov_base);
  //if (iph->ip_sum == 0)
  iph->ip_sum = in_cksum(iph, sizeof(struct ip));
  
  static char strSrc[16];
  static char strDst[16];
  int msgLen = iov[0].iov_len + iov[1].iov_len;
  ...
  stream_put(s, iov[0].iov_base, iov[0].iov_len);  
  stream_put(s, iov[1].iov_base, iov[1].iov_len);  
  stream_putw_at (s, 0, stream_get_endp (s));    
  
  if (zclient_send_message(zclient) != 0)
  {
    inet_ntop(AF_INET, src, strSrc, 15);
    inet_ntop(AF_INET, dst, strDst, 15);
    zlog_err("ospf_sendmsg: Failed to send message (%s -> %s) ifindex %d, flags %d, msg len %d", strSrc, strDst, ifindex, flags, msgLen);
  }  
  return msgLen;
}
#endif

static int ospf_write_create_and_send_packet(struct ospf *ospf, struct ospf_interface *oi, uint16_t maxdatasize, struct ospf_packet *op)
{
  int ret;
  u_char type;
  ...
  {
    zlog_err("ospf_write_frags: Not implemented");
    ospf_write_frags (ospf->fd, op, &iph, iov, maxdatasize, oi->ifp->mtu, flags, type);
  }
#endif /* WANT_OSPF_WRITE_FRAGMENT */
#endif
...
#ifndef HAVE_ZEBRA_CPA
  ret = ospf_sendmsg(ospf->fd, &oi->address->u.prefix4, &op->dst, oi->ifp->ifindex, iov, flags);
#else
  ret = ospf_sendmsg(          &oi->address->u.prefix4, &op->dst, oi->ifp->ifindex, iov, flags);  
#endif
  sockopt_iphdrincl_swab_systoh (&iph);
  
  if (ret < 0)
    zlog_warn ("*** sendmsg in ospf_write failed to %s, id %d, off %d, len %d, interface %s, mtu %u: %s", inet_ntoa (iph.ip_dst), iph.ip_id, iph.ip_off, iph.ip_len, oi->ifp->name, oi->ifp->mtu, safe_strerror (errno));
...
#ifndef HAVE_ZEBRA_CPA  
//TODO Adam: Use zebra to send packets
static int ospf_write (struct thread *thread)
{
  ...
  ospf_write_create_and_send_packet(ospf, oi, maxdatasize, op);
...
#else
static int ospf_write (struct ospf_packet *op, struct ospf_interface *oi)
{
  u_int16_t maxdatasize = 1000; //TODO fixme

  ospf_write_create_and_send_packet(oi->ospf, oi, maxdatasize, op);
...
static void ospf_hello (struct ip *iph, struct ospf_header *ospfh, struct stream * s, struct ospf_interface *oi, int size)
{
  ...
  if (IPV4_ADDR_SAME (&ospfh->router_id, &oi->ospf->router_id))
  {
    ...
    {
      zlog_debug ("ospf_header[%s/%s]: selforiginated, dropping.", LOOKUP (ospf_packet_type_str, ospfh->type), inet_ntoa (iph->ip_src));
    }
    return;
  }
...
int ospf_read_process(struct stream *ibuf, struct interface *ifp, struct ospf *ospf)
{
  int ret;
  struct ospf_interface *oi;
  struct ip *iph;
  ...
  /* Note that there should not be alignment problems with this assignment
     because this is at the beginning of the stream data buffer. */
  iph = (struct ip *) (STREAM_DATA (ibuf) + stream_get_getp(ibuf));
  ...
  if (oi && OSPF_IF_PASSIVE_STATUS (oi) == OSPF_IF_PASSIVE)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("ignoring packet from router %s sent to %s, received on a passive interface, %s",
		    inet_ntop(AF_INET, &ospfh->router_id, buf[0], sizeof(buf[0])),
		    inet_ntop(AF_INET, &iph->ip_dst, buf[1], sizeof(buf[1])),
		    inet_ntop(AF_INET, &oi->address->u.prefix4, buf[2], sizeof(buf[2])));
...
ospf_read (struct thread *thread)
{
  ...
  return ospf_read_process(ibuf, ifp, ospf);
}
#endif
...
ospf_db_desc_resend (struct ospf_neighbor *nbr)
{
  ...
  OSPF_ISM_WRITE_ON (oi->ospf);
#else
  ospf_write(ospf_packet_dup (nbr->last_send), oi);
#endif
