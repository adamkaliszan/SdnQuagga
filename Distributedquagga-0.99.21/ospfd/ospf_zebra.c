...
static int
ospf_interface_add (int command, struct zclient *zclient, zebra_size_t length, uint16_t virtualNetworkNo)
{
  ...
  if (if_is_operative (ifp))
    {
      ...
      zebra_interface_if_set_value (zclient->ibuf, ifp, virtualNetworkNo);
  ...
  if (zclient->redist[ZEBRA_ROUTE_OSPF])
    {
      ...
      zclient_create_header (s, ZEBRA_IPV4_ROUTE_ADD, 0); //TODO Adam zadbać o pozostałe instancje
      ...
      for (ALL_LIST_ELEMENTS (or->paths, node, nnode, path))
        {
          ...
          zapi_ipv4_route (ZEBRA_IPV4_ROUTE_DELETE, zclient, p, &api, 0); //TODO Adam zadbać o pozostałe instancje
  ...
  if (zclient->redist[ZEBRA_ROUTE_OSPF])
    {
      ...
      zapi_ipv4_route (ZEBRA_IPV4_ROUTE_ADD, zclient, p, &api, 0); //TODO Adam zadbać o pozostałe instancje
...
static int ospf_zebra_read_txc(int command, struct zclient *zclient, zebra_size_t length)
{
  struct stream *s;
  struct zapi_ipv4 api;
...
ospf_zebra_init ()
{
  ...
  zclient->txc = ospf_zebra_read_txc;
