...
ssize_t 
stream_read_try_IPv6(int fd, struct in6_addr *src, struct in6_addr *dst, int *ifindex, unsigned int *virtualNetworkNo, struct iovec *message)
{
  *virtualNetworkNo = 0;
  ...
  if (retval < 0)
    zlog_err("recvmsg failed: %s", strerror (errno));
  //else if (retval == iov_totallen (message))
  //  errorLog("recvmsg read full buffer size: %d", retval);
...
ssize_t 
stream_read_try_IPv4(int fd, struct in_addr *src, struct in_addr *dst, int *ifindex, unsigned int *virtualNetworkNo, struct iovec *message)
{
  *virtualNetworkNo = 0;
