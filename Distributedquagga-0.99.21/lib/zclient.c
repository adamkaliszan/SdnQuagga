...
zclient_socket(void)
{
  ...
  if ( zclient_serv_path == NULL)
  {
    ...
    zlog_debug("Connecting to zebra on localhost");
  }
  else
  {
    inet_aton(zclient_serv_path, &serv.sin_addr);
    zlog_debug("Connecting to zebra on %s", zclient_serv_path);
  }
  /* Connect to zebra. */
...
zclient_send_message(struct zclient *zclient)
{
  if (zclient->sock < 0)
  {
    zlog_err("NOT connected to zebra server, can't send message");
    return -1;
  }
...
void
zclient_create_header (struct stream *s, uint16_t command, uint16_t virtualNetworkNo)
{
...
static int
zebra_message_send (struct zclient *zclient, int command, uint16_t virtualNetworkNo)
{
  struct stream *s;
  ...
  zclient_create_header (s, command, virtualNetworkNo);
  ...
  if (zclient->redist_default)
    {
      ...
      zclient_create_header (s, ZEBRA_HELLO, 0);
...
int zclient_start (struct zclient *zclient)
{
  ...
  zebra_message_send (zclient, ZEBRA_ROUTER_ID_ADD, 0); //TODO Adam zadbać o pozostałe instancje

  /* We need interface information. */
  zebra_message_send (zclient, ZEBRA_INTERFACE_ADD, 0); //TODO Adam zadbać o pozostałe instancje

  /* Flush all redistribute request. */
  for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
    if (i != zclient->redist_default && zclient->redist[i])
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, i, 0); //TODO Adam zadbać o pozostałe instancje

  /* If default information is needed. */
  if (zclient->default_information)
    zebra_message_send (zclient, ZEBRA_REDISTRIBUTE_DEFAULT_ADD, 0); //TODO Adam zadbać o pozostałe instancje
  
  if (zclient->NetworkTxcInit != NULL)
    zclient->NetworkTxcInit();  
...
zapi_ipv4_route (u_char cmd, struct zclient *zclient, struct prefix_ipv4 *p,
                 struct zapi_ipv4 *api, uint16_t virtualNetworkNo)
{
  int i;
  int psize;
  struct stream *s;
  ...
  zclient_create_header (s, cmd, virtualNetworkNo);
...
zapi_ipv6_route (u_char cmd, struct zclient *zclient, struct prefix_ipv6 *p,
	       struct zapi_ipv6 *api, uint16_t virtualNetworkNo)
{
  int i;
  int psize;
  struct stream *s;
 ...
 */
int
zebra_redistribute_send (int command, struct zclient *zclient, int type, uint16_t virtualNetworkNo)
{
  struct stream *s;
...
struct interface *
zebra_interface_add_read (struct stream *s, uint16_t virtualNetworkNo)
{
  ...
  zebra_interface_if_set_value (s, ifp, virtualNetworkNo);
 ...
 */
struct interface *
zebra_interface_state_read (struct stream *s, uint16_t virtualNetworkNo)
{
  struct interface *ifp;
  char ifname_tmp[INTERFACE_NAMSIZ];
...
void
zebra_interface_if_set_value (struct stream *s, struct interface *ifp, uint16_t virtualNetworkNo)
{
  ifp->virtualNetworkNo = virtualNetworkNo;
...
zclient_read (struct thread *thread)
{
  ...
  virtualNetworkNo = stream_getw (zclient->ibuf);
  ...
  switch (command)
	...
	(*zclient->router_id_update) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_ADD:
      if (zclient->interface_add)
	(*zclient->interface_add) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_DELETE:
      if (zclient->interface_delete)
	(*zclient->interface_delete) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_ADDRESS_ADD:
      if (zclient->interface_address_add)
	(*zclient->interface_address_add) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_ADDRESS_DELETE:
      if (zclient->interface_address_delete)
	(*zclient->interface_address_delete) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_UP:
      if (zclient->interface_up)
	(*zclient->interface_up) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_INTERFACE_DOWN:
      if (zclient->interface_down)
	(*zclient->interface_down) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV4_ROUTE_ADD:
      if (zclient->ipv4_route_add)
	(*zclient->ipv4_route_add) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV4_ROUTE_DELETE:
      if (zclient->ipv4_route_delete)
	(*zclient->ipv4_route_delete) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV6_ROUTE_ADD:
      if (zclient->ipv6_route_add)
	(*zclient->ipv6_route_add) (command, zclient, length, virtualNetworkNo);
      break;
    case ZEBRA_IPV6_ROUTE_DELETE:
      if (zclient->ipv6_route_delete)
	(*zclient->ipv6_route_delete) (command, zclient, length, virtualNetworkNo);
      break;
#ifdef HAVE_ZEBRA_CPA
    case ZEBRA_TXC:
      if (zclient->txc)
        (*zclient->txc)(command, zclient, length, virtualNetworkNo);
      break;
#endif
    default:
      break;
...
zclient_redistribute (int command, struct zclient *zclient, int type)
{
  ...
  if (zclient->sock > 0)
    zebra_redistribute_send (command, zclient, type, 0); //TODO Adam uzupełnić numer wirtualnej sieci
}
...
zclient_redistribute_default (int command, struct zclient *zclient)
{
  ...
  if (zclient->sock > 0)
    zebra_message_send (zclient, command, 0); //TODO Adam zadpać o podanie nr wirtualnej sieci
}
...
#ifdef HAVE_TCP_ZEBRA 
  struct in_addr tmp;
  if (inet_aton(path, &tmp) == 0)
  {
    zlog_warn ("%s: wrong IP address `%s'", __func__, inet_ntoa(tmp));
    return;    
  }
#else
  struct stat sb;
